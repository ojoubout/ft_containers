...
public:
    ...
    reverse_iterator    rbegin() {
        if (_last_chunk_size == 0)
            return (reverse_iterator(_map + _map_size, 0));
        return (reverse_iterator(_map + _map_size - 1, _last_chunk_size));
    }
    const_reverse_iterator    rend() const {
        return (const_reverse_iterator(_map, 0));
    }

    const_reverse_iterator    rbegin() const {
        if (_last_chunk_size == 0)
            return (const_reverse_iterator(_map + _map_size, 0));
    ...
    void resize (size_type n, value_type val = value_type()) {
        size_type   map_size = (n + BUFF_SIZE - 1) / BUFF_SIZE; // ceil(n / BUFF_SIZE)
        ...
        if (map_size == _map_size) {
            ...
            map_pointer map = new pointer[map_size + 1];
            map[map_size] = _map[_map_size];
            ...
            _map[map_size] = _map[_map_size];
        }
        _map_size = map_size;
        _last_chunk_size = last_chunk_size == 0 ? BUFF_SIZE : last_chunk_size;
    }
